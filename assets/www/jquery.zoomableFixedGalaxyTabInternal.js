(function ($) {
  $.fn.zoomableInternal = function (method,id) {
    ...
    return this.each(function (index, value) {
	  ...
	  var init = function() {
		if (value.useMap != "") {
		  var temporiginalMapViews = document.getElementById("originalMapInternals");
      ...
      var zoomMap = function () {
        ...
        if (map != null) {
          for (var i = 0; i < map.areas.length; i++) {
            ...
            if(dojo.isIE){
                if(!zoomData.isSetIE){
                    dojo.style(dojo.byId('internalcallout_'+i), {top: (coords[1]-4)+"px"});
                    zoomData.isSetIE = true;
                }else{
                    dojo.style(dojo.byId('internalcallout_'+i), {top: (value.offsetTop+coords[1]-4)+"px"});
                }
                dojo.style(dojo.byId('internalcallout_'+i), {left: (value.offsetLeft+coords[0]-4)+"px"});
            }else{
                dojo.style(dojo.byId('internalcallout_'+i), {top: (value.offsetTop+coords[1]-4)+"px"});
                dojo.style(dojo.byId('internalcallout_'+i), {left: (value.offsetLeft+coords[0]-4)+"px"});
            }
            
            dojo.style(dojo.byId('internalcallout_'+i), {width: (coords[2]-coords[0]+8)+"px"});
            dojo.style(dojo.byId('internalcallout_'+i), {height: (coords[3]-coords[1]+8)+"px"});
